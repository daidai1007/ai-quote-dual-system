...
FROM calc.cabinet_auxiliary_catalog_version WHERE data_version='cabinet-auxiliary-189ae4efbfcb40f5-v3';
...
WHERE p.data_version='cabinet-auxiliary-189ae4efbfcb40f5-v3' GROUP BY p.product_code ORDER BY p.product_code;
SELECT product_code,count(*) AS fixed_rules FROM calc.cabinet_auxiliary_fixed_rule
WHERE data_version='cabinet-auxiliary-189ae4efbfcb40f5-v3' GROUP BY product_code ORDER BY product_code;
...
WHERE p.data_version='cabinet-auxiliary-189ae4efbfcb40f5-v3'
...
WHERE p.data_version='cabinet-auxiliary-189ae4efbfcb40f5-v3' AND l.quantity_rule->>'kind' NOT IN ('CONSTANT','HEIGHT_GT');
COMMIT;
