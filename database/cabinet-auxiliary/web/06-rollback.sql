...
BEGIN
  ...
  WHERE status='RETIRED' AND data_version<>'cabinet-auxiliary-fef2b7a93d50e60c-v3' ORDER BY created_at DESC LIMIT 1;
  IF v_previous IS NULL THEN RAISE EXCEPTION 'No previous RETIRED auxiliary version is available'; END IF;
  UPDATE calc.cabinet_auxiliary_catalog_version SET status='RETIRED',activated_at=NULL WHERE data_version='cabinet-auxiliary-fef2b7a93d50e60c-v3' AND status='ACTIVE';
