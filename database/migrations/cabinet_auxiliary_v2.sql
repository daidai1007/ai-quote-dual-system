-- Complete versioned cabinet auxiliary catalog.
...
CREATE OR REPLACE FUNCTION calc.stage_cabinet_auxiliary_catalog_v2(p_payload jsonb)
...
BEGIN
  IF v_version IS NULL OR v_version!~'^cabinet-auxiliary-[0-9a-f]{16}-v2$' THEN RAISE EXCEPTION 'Invalid cabinet auxiliary V2 data_version'; END IF;
  ...
  IF v_count<>255 THEN RAISE EXCEPTION 'Staged auxiliary line count %, expected 255',v_count; END IF;
  ...
  RETURN v_version;
END $$;
REVOKE ALL ON FUNCTION calc.stage_cabinet_auxiliary_catalog_v2(jsonb) FROM PUBLIC;

CREATE OR REPLACE FUNCTION calc.activate_cabinet_auxiliary_catalog_v2(p_version text)
RETURNS text LANGUAGE plpgsql SECURITY INVOKER SET search_path=pg_catalog,calc AS $$
...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM calc.cabinet_auxiliary_catalog_version WHERE data_version=p_version AND status='STAGED')
     OR v_profiles<>16 OR v_lines<>255 OR v_fixed<>72
    THEN RAISE EXCEPTION 'Version % is not a complete STAGED auxiliary V2 catalog',p_version; END IF;
  ...
  RETURN p_version;
END $$;
REVOKE ALL ON FUNCTION calc.activate_cabinet_auxiliary_catalog_v2(text) FROM PUBLIC;

-- Keep the DB-owned base quote function valid after the legacy auxiliary tables
-- are removed. The API replaces this base value with the dynamic V2 result.
