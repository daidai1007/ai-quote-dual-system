/*
 * Exact rollback for the width-first quick-quote migration.
 * Restores the immediately previous rule: perimeter first, then 3-D distance.
 * Door type and model text remain excluded from face-price selection.
 * No quote-experience rows are inserted, updated or deleted.
 */
BEGIN;
...
AS $function$
  WITH canonical AS (
    ...
    ORDER BY perimeter_distance,distance,range_rank,effective_from DESC,quick_rule_id DESC
    LIMIT 1
  )
