...
test('versioned cabinet catalog stages, activates, calculates and saves an immutable snapshot',async()=>{
  ...
  `);
  runFile('database/migrations/cabinet_material_v1.sql');
  runFile('database/migrations/cabinet_spray_v1.sql');
  runFile('database/migrations/cabinet_auxiliary_v1.sql');
  runFile('database/migrations/cabinet_labor_v1.sql');
  const staged=runFile('database/cabinet-material/web/02-stage.sql');
  assert.match(staged,/cabinet-material-91cc0a5f841b7455-v1/);
  ...
  assert.equal(sql(`SELECT calc.activate_cabinet_material_catalog_v1('cabinet-material-91cc0a5f841b7455-v1');`),'cabinet-material-91cc0a5f841b7455-v1');
  const sprayStaged=runFile('database/cabinet-spray/web/02-stage.sql');
  assert.match(sprayStaged,/cabinet-spray-13f257f440859e3b-v1/);
  assert.equal(sql(`SELECT count(*) FROM calc.cabinet_spray_rule;`),'202');
  assert.equal(sql(`SELECT calc.activate_cabinet_spray_catalog_v1('cabinet-spray-13f257f440859e3b-v1');`),'cabinet-spray-13f257f440859e3b-v1');
  const auxiliaryStaged=runFile('database/cabinet-auxiliary/web/02-stage.sql');
  assert.match(auxiliaryStaged,/cabinet-auxiliary-1dc400290ae91924-v1/);
  assert.equal(sql(`SELECT count(*) FROM calc.cabinet_auxiliary_line;`),'255');
  assert.equal(sql(`SELECT calc.activate_cabinet_auxiliary_catalog_v1('cabinet-auxiliary-1dc400290ae91924-v1');`),'cabinet-auxiliary-1dc400290ae91924-v1');
  ...
  assert.equal(material.data_version,'cabinet-material-91cc0a5f841b7455-v1');
  ...
  try{
    ...
    assert.equal(body.formula_cost.cabinet_spray_version,'cabinet-spray-13f257f440859e3b-v1');
    assert.equal(body.formula_cost.cabinet_auxiliary_version,'cabinet-auxiliary-1dc400290ae91924-v1');
    ...
    assert.equal(sql(`SELECT cabinet_spray_version FROM calc.dual_quote_result WHERE quote_id='HTTP-CABINET';`),'cabinet-spray-13f257f440859e3b-v1');
    assert.equal(sql(`SELECT cabinet_auxiliary_version FROM calc.dual_quote_result WHERE quote_id='HTTP-CABINET';`),'cabinet-auxiliary-1dc400290ae91924-v1');
